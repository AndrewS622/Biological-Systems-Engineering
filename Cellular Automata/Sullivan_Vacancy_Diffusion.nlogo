globals [EnergyL EnergyR delGL delGR VacConcR VacConcL VacanciesR VacanciesL num-green num-blue-left num-blue-right x y nblue ngreen pblue prob atom]
breed [atoms-green atom-green]
breed [atoms-blue atom-blue]
breed [atoms-right atom-right]

to setup
  clear-all
  set-default-shape turtles "circle"

  set delGL 1.29 * 1.6 * 10 ^ -19
  set delGR delGL
  set EnergyL (delGL / (1.38 * Temperature * 10 ^ -23))
  set EnergyR (delGR / (1.38 * Temperature * 10 ^ -23))
  set VacConcL exp(-1 * EnergyL)
  set VacConcR exp(-1 * EnergyR)
  set VacanciesL floor(VacConcL * (2 * max-pycor + 1)*(max-pxcor + 1))
  set VacanciesR floor(VacConcR * (2 * max-pycor + 1)*(max-pxcor))

  set num-blue-left floor(Concentration * (max-pxcor + 1)*(2 * max-pycor + 1))
  Create-atoms-blue num-blue-left[
    setxy random (min-pxcor - 1) random-pycor
    loop[
      ifelse (any? other atoms-blue-here) = TRUE[
      setxy random(min-pxcor - 1) random-pycor
      ]
      [stop]
    ]
  ...
  set num-green ((max-pxcor + 1) * (2 * max-pycor + 1) - num-blue-left - VacanciesL)
  Create-atoms-green num-green[
    setxy random(min-pxcor - 1) random-pycor
    loop[
      ifelse (any? other atoms-green-here) or (any? atoms-blue-here) = TRUE[
      setxy random(min-pxcor - 1) random-pycor
      ]
      [stop]
    ]
  ...
  set num-blue-right ((max-pxcor) * (2 * max-pycor + 1) - VacanciesR)
  Create-atoms-right num-blue-right[
    setxy random max-pxcor + 1 random-pycor
    loop[
      ifelse (any? other atoms-right-here) = TRUE[
      setxy random max-pxcor + 1 random-pycor
      ]
      [stop]
    ]
...
to move-atom-to-here  ;; patch procedure
  set nblue count (turtles-on neighbors4) with ([color = blue])
  set ngreen count (turtles-on neighbors4) with ([color = green])
  set pblue ifelse-value (nblue + ngreen = 0) [-1] [nblue * DiffusionRatio / (nblue + ngreen)]
  set prob random-float 1
  ifelse (pblue = -1)[stop][
    ifelse (ngreen = 0) [set atom one-of (turtles-on neighbors4) with ([color = blue])
    ask atom [move-to myself]
    stop
  ][
    set atom ifelse-value (prob >= pblue) [one-of (turtles-on neighbors4) with ([color = green])] [one-of (turtles-on neighbors4) with ([color = blue])]
...
to plot-atoms [atoms]
  ...
  repeat world-width [
    let n count atoms with [pxcor = column]
    plotxy column n
    plot-pen-down
    set column column + 1
    plotxy column n
